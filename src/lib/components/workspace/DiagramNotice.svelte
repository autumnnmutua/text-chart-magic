<script lang="ts">
  ...
  import { diagramNotice, dismissDiagramNotice } from '$lib/util/diagramNotice.svelte';
...
{#if diagramNotice.message}
  <div
    ...
    <Button
      class="pointer-events-auto -mt-1 -mr-2 size-8 shrink-0"
      size="icon"
      variant="ghost"
      aria-label="关闭操作提示"
      onclick={dismissDiagramNotice}>
      <X class="size-4" />
    </Button>
  </div>
{/if}
